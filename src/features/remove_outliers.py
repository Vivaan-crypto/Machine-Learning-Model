...
outlierColumns = list(dataFrame.columns[:6])
# --------------------------------------------------------------
# Plotting outliers
# --------------------------------------------------------------
...
dataFrame[outlierColumns[:3] + ["label"]].boxplot(
...
dataFrame[outlierColumns[:3] + ["label"]].plot.hist(
...
dataset, outliers, X_scores = mark_outliers_lof(dataFrame, outlierColumns)
for col in outlierColumns:
...
dataset, outliers, X_scores = mark_outliers_lof(
    dataFrame[dataFrame["label"] == label], outlierColumns
)
for col in outlierColumns:
    plot_binary_outliers(dataset, col, "outlier_lof", reset_index=True)
...
# Create a loop
outlierRemovedDataFrame = dataFrame.copy()
for col in outlierColumns:
    for label in dataFrame["label"].unique():
        dataset = mark_outliers_chauvenet(dataFrame[dataFrame["label"] == label], col)

        # replace value marked as an outlier to NaN
        dataset.loc[dataset[col + "_outlier"], col] = np.nan
        # Update the column inside the original dataframe
        outlierRemovedDataFrame.loc[
            (outlierRemovedDataFrame["label"] == label), col
        ] = dataset[col]
        # This variable stores how many outliers there were in each column
        n_outliers = len(dataFrame) - len(outlierRemovedDataFrame[col].dropna())
        print(f"Removed {n_outliers} from {col} for {label}")
outlierRemovedDataFrame.info()
# --------------------------------------------------------------
# Export new dataframe
# --------------------------------------------------------------
outlierRemovedDataFrame.to_pickle(
